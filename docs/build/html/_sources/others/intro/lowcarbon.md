...
### Sector-Consistent Decarbonization

In the previous sub-section, the portfolio decarbonization is conducted with other constraint than decreasing the portfolio's WACI.

However, we have seen in the previous part that a few sectors are responsible for most of energy-related emissions, that is a good first-order approximation of anthropogenic carbon emissions.

A sector-consistent portfolio decarbonization should ensure that high-emitting stocks are excluded within each of these impactful sectors, rather than switching Utilities stocks for Information & Technology ones for example.

To do so, we can impose a sector neutrality (we follow Roncalli (2023) as a reference for sector neutrality in the optimization problem), by keeping the sectors weights the same as the initial benchmark. 
...
In the QP problem, this can be included in the $Ax = b$ equality constraint. We have:

\begin{equation}
A_{Energy} = s_{Energy}^T
\end{equation}
 
\begin{equation}
A_{Utilities} = s_{Utilities}^T
\end{equation}

\begin{equation}
A_{Cement} = s_{Cement}^T
\end{equation}
...
\begin{equation}
A_{Chemicals} = s_{Chemicals}^T
\end{equation}

\begin{equation}
A_{Textile \; \& \; Leather} = s_{Textile \; \& \; Leather}^T
\end{equation}

\begin{equation}
A_{Aluminium} = s_{Aluminium}^T
\end{equation}

\begin{equation}
A_{Food \; Processing} = s_{Food \; Processing}^T
\end{equation}

\begin{equation}
A_{Transport} = s_{Transport}^T
\end{equation}

\begin{equation}
A_{Others} = s_{Others}^T
\end{equation}
...
And $b_{Energy} = s_{Energy}^Tb$, $b_{Cement} = s_{Cement}^Tb$, $b_{Steel} = s_{Steel}^Tb$, and so on, such that:


\begin{equation}
b = \begin{bmatrix}
b_{Energy} + b_{Utilities} \\
b_{Cement} \\
b_{Steel} \\
b_{Chemicals} \\
b_{Textile \; \& \; Leather} \\
